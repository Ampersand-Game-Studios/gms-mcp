...
class TestIntrospectionBasics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.yyp_data = {
            ...
            "TextureGroups": [{"name": "Default", "autocrop": True, "border": 2}],
...
""")
            ...
            json.dump(
                {
                    ...
                    "files": [{"functions": [{"name": "steam_init"}]}],
                },
                f,
            )
    ...
    def test_list_assets_combined_filters(self):
        ...
        # Type + Name
        assets = list_assets_by_type(self.project_root, asset_type_filter="object", name_contains="player")
        self.assertEqual(len(assets.get("object", [])), 1)
        self.assertNotIn("sprite", assets)

        # Type + Folder
        assets = list_assets_by_type(self.project_root, asset_type_filter="script", folder_prefix="scripts")
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_missing_asset_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            yyp = {"resources": [{"id": {"name": "missing_asset", "path": "objects/missing/missing.yy"}}]}
