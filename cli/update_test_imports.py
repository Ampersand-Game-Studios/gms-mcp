...
def update_imports_in_file(file_path: Path, dry_run: bool = False, verbose: bool = False) -> Tuple[bool, List[str]]:
    ...
    legacy_path_patterns = [
        ...
        (r"sys\.path\.insert\(0, str\(Path\(__file__\)\.parent\)\)", "sys.path.insert(0, str(PROJECT_ROOT))"),
        (r"# Add parent directory to path", "# Add project root to path"),
    ]
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Bulk update test imports to use correct package paths")
