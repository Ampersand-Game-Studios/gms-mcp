...
class TestRealGameMakerSmoke(unittest.TestCase):
    def test_compile_diagnostics_allowlist_categories_without_publishing_output(self):
        result = {
            "error": {
                "details": {
                    "compile_verification": {
                        "exit_code": 127,
                        "compile_stage_ok": False,
                        "timed_out": False,
                        "attempt_count": 1,
                        "stdout_tail": "/private/project/compiler: error while loading shared libraries: libExample.so.1",
                        "stderr_tail": "System.DllNotFoundException for private@example.com",
                    }
                }
            }
        }

        diagnostics = real_smoke._safe_compile_diagnostics(result)
        report = real_smoke._certification_report(
            {"ok": False, "status": "failed", "diagnostics": diagnostics},
            fixture={"name": "gm-2024", "host_platform": "linux"},
            runtime={"ok": True, "version": "2024.14.4.268"},
        )
        serialized = json.dumps(report)

        self.assertEqual(report["diagnostics"]["category"], "missing_shared_library")
        self.assertEqual(report["diagnostics"]["missing_library"], "libExample.so.1")
        self.assertEqual(report["diagnostics"]["exception_type"], "System.DllNotFoundException")
    ...
    def test_synthetic_fixtures_are_deterministic_and_studio_free(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            first = real_smoke.create_synthetic_project(
                root / "first",
                project_name="gms_mcp_smoke_2026_lts",
                ide_version="2026.0.0.16",
            )
            second = real_smoke.create_synthetic_project(
                root / "second",
                project_name="gms_mcp_smoke_2026_lts",
                ide_version="2026.0.0.16",
            )
            ...
            room_bytes = (first.parent / "rooms/r_mcp_smoke/r_mcp_smoke.yy").read_bytes()
            project = json.loads(first_bytes)
            room = json.loads(room_bytes)

        self.assertEqual(first_bytes, second_bytes)
        self.assertNotIn(b"\r", first_bytes)
        self.assertNotIn(b"\r", room_bytes)
        self.assertEqual(project["name"], "gms_mcp_smoke_2026_lts")
        self.assertEqual(project["IncludedFiles"], [])
        self.assertEqual(project["ForcedPrefabProjectReferences"], [])
        self.assertEqual(
            project["resources"], [{"id": {"name": "r_mcp_smoke", "path": "rooms/r_mcp_smoke/r_mcp_smoke.yy"}}]
        )
    ...
    def test_main_runs_smart_verification_sequence_against_project_copy(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            reference_yyp = real_smoke.create_synthetic_project(
                temp_path / "reference",
                project_name="gms_mcp_smoke_2026_lts",
                ide_version="2026.0.0.16",
            )
            expected_hash = hashlib.sha256(reference_yyp.read_bytes()).hexdigest()
            argv = [
        ...
        self.assertEqual(report["fixture"]["host_platform"], "linux")
        self.assertEqual(report["fixture"]["expected_runtime_version"], "runtime-*")
        self.assertEqual(report["fixture"]["source_yyp_name"], "gms_mcp_smoke_2026_lts.yyp")
        self.assertEqual(report["fixture"]["source_yyp_sha256"], expected_hash)
        self.assertEqual(report["fixture"]["source_ide_version"], "2026.0.0.16")
