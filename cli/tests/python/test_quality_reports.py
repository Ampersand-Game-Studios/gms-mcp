...
class TestQualityReportParsing(unittest.TestCase):
    def test_registered_tool_discovery_uses_a_disposable_pinned_project(self):
        with patch.dict(
            os.environ,
            {"GM_PROJECT_ROOT": "", "PROJECT_ROOT": str(PROJECT_ROOT)},
            clear=False,
        ):
            specs = quality_reports.discover_registered_mcp_tools("core")

        names = {spec.name for spec in specs}
        self.assertIn("gm_project_info", names)
        self.assertIn("gm_capabilities", names)
