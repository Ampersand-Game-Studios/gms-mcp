...
class RunnerDiscoveryMixin:
    ...
    def find_license_file(self) -> Optional[Path]:
        ...
        explicit_license = os.environ.get("GMS_MCP_LICENSE_FILE")
        if explicit_license is not None:
            # CI supplies one exact ephemeral file. If it is missing, fail closed
            # instead of searching the rest of the machine for another licence.
            license_path = Path(explicit_license).expanduser()
            return license_path if license_path.is_file() else None
