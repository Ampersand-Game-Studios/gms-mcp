...
```
game-project/            <- Project Root (repo root)
...
# SAFETY: Skipped companion files in a directory containing multiple assets
# Protected companion files from accidental deletion
```
...
**Example Error**:
```
File missing for GMSprite spr_example:
sprites\spr_example\layers\[frame_uuid]\[layer_uuid].png
```
