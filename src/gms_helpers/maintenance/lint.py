...
class LintIssue:
    ...
    def to_diagnostic(self) -> Diagnostic:
        ...
        return Diagnostic(
            ...
            suggested_fix="Run 'gms maintenance fix-issues' to attempt an automatic fix."
            if self.category == "json"
            else None,
        )
    ...
    def _scan_yy_files(self):
        ...
        for yy_file in yy_files:
            ...
            try:
                ...
                self.issues.append(
                    LintIssue(severity="error", category="json", file_path=yy_file, message=f"Invalid JSON: {e}")
                )
    ...
    def _validate_naming_conventions(self):
        ...
        for resource in resources:
            ...
            try:
                ...
                self.issues.append(LintIssue(severity="warning", category="naming", file_path=path, message=str(e)))
    ...
    def _validate_project_structure(self):
        ...
        for resource in resources:
            ...
            if path and not path.startswith("folders/") and not os.path.exists(path):
                self.issues.append(
                    LintIssue(
                        severity="error", category="reference", file_path=path, message="Referenced file does not exist"
                    )
                )
