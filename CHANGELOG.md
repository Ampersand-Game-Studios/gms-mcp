...
- **Local Streamable HTTP**: `gms-mcp server --transport streamable-http` serves a stateless, bearer-authenticated local endpoint, rejects non-loopback binds and unapproved Host/Origin headers, caps request bodies at 1 MiB, and validates standard and parameter-routing MCP headers. This remains deliberately local-only rather than a remote shared-server mode.
