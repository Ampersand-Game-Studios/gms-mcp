...
class ReferenceScanner:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if old_name in line:
                    ...
                    if (
                        '"name"' in line
                        and f'"{old_name}"' in line
                        and '"$GMSequence"' in content[max(0, content.find(line) - 200) : content.find(line) + 200]
                    ):
                        new_line = line.replace(f'"{old_name}"', f'"{new_name}"')
                        self.references.append(
                            AssetReference(
                    ...
                    if '"path"' in line and f"sprites/{old_name}/{old_name}.yy" in line:
                        new_line = line.replace(
                            f"sprites/{old_name}/{old_name}.yy", f"sprites/{new_name}/{new_name}.yy"
                        )
                        self.references.append(
                            AssetReference(
                                file_path=sprite_yy,
                ...
                for ref in refs_sorted:
                    if ref.line_number <= len(lines):
                        ...
                        lines[ref.line_number - 1] = lines[ref.line_number - 1].replace(
                            ref.old_text.strip(), ref.new_text.strip()
                        )
    ...
    def validate_no_stale_references(self, old_name: str) -> List[str]:
        ...
        search_patterns = ["*.yyp", "*.resource_order", "*.yy", "*.gml"]
