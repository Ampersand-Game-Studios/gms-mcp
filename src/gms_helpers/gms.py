...
{help_epilog}
    ...
    parser.add_argument(
        "--project-root",
        default=".",
        help="Project root directory (directory containing .yyp, or repo root containing gamemaker/)",
    )
    parser.add_argument(
        "--telemetry",
...
def setup_skills_commands(subparsers):
    ...
    install_parser = skills_subparsers.add_parser(
        "install", help="Install skills to Claude (default) or OpenClaw directories"
    )
    ...
    install_parser.add_argument("--force", action="store_true", help="Overwrite existing skill files")
    ...
    list_parser.add_argument(
        "--openclaw", action="store_true", help="Inspect OpenClaw skills directories instead of Claude directories"
    )
    list_parser.add_argument("--installed", action="store_true", help="Only show installed skills")
...
def setup_telemetry_commands(subparsers):
    ...
    enable_parser.add_argument(
        "--with-install-id", action="store_true", help="Also include a stable hashed install identifier"
    )
...
def setup_doc_commands(subparsers):
    ...
    lookup_parser.add_argument("--refresh", action="store_true", help="Bypass cache and fetch fresh documentation")
    ...
    clear_parser.add_argument(
        "--functions-only", action="store_true", help="Only clear cached functions, keep the index"
    )
...
def setup_diagnostics_commands(subparsers):
    ...
    parser.add_argument("--depth", default="quick", choices=["quick", "deep"], help="Search depth (default: quick)")
...
def setup_script_parser(subparsers):
    ...
    parser.add_argument(
        ...
        default=True,
        help="Show verbose maintenance output",
    )
...
def setup_font_parser(subparsers):
    ...
    parser.add_argument(
        "--uses-sdf", action=argparse.BooleanOptionalAction, default=True, help="Use SDF rendering (default: True)"
    )
    parser.add_argument("--skip-maintenance", action="store_true", help="Skip pre/post validation")
    parser.add_argument("--no-auto-fix", action="store_true", help="Do not automatically fix issues")
    parser.add_argument(
...
def setup_texture_group_commands(subparsers):
    ...
    assign_parser.add_argument(
        "--name-contains", dest="name_contains", help="Filter by name substring (case-insensitive)"
    )
    ...
    assign_parser.add_argument(
        "--no-top-level", dest="no_top_level", action="store_true", help="Do not change top-level textureGroupId"
    )
    assign_parser.add_argument(
...
def setup_maintenance_commands(subparsers):
    ...
    auto_parser.add_argument(
        "--verbose", action=argparse.BooleanOptionalAction, default=True, help="Show detailed output"
    )
    ...
    validate_parser.add_argument(
        "--strict-disk-check", action="store_true", help="Also check that folder .yy files exist on disk"
    )
    validate_parser.add_argument(
    ...
    dedupe_parser.add_argument(
        "--auto", action="store_true", help="Automatically keep first occurrence of each duplicate"
    )
    dedupe_parser.add_argument(
    ...
    clean_orphans_parser.add_argument(
        "--skip-types", nargs="*", default=["folder"], help="Asset types to skip during cleanup"
    )
...
def setup_symbol_commands(subparsers):
    ...
    list_parser.add_argument(
        "--kind", choices=["function", "enum", "macro", "globalvar", "constructor"], help="Filter by symbol kind"
    )
...
from .commands.maintenance_commands import (
    ...
    handle_maintenance_health,
)
from .commands.runner_commands import handle_runner_compile, handle_runner_run, handle_runner_stop, handle_runner_status
...
from .commands.skills_commands import handle_skills_install, handle_skills_list, handle_skills_uninstall
