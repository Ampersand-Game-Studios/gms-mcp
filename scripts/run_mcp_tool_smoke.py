...
class MCPToolSmokeRunner:
    ...
    async def run(self) -> int:
        ...
        for index, tool_name in enumerate(selected_tools, start=1):
            ...
            # A production MCP server is intentionally pinned to one project
            # for its lifetime. Each isolated smoke workspace therefore gets
            # its own server process-equivalent instance.
            os.environ["GM_PROJECT_ROOT"] = str(workspace)
