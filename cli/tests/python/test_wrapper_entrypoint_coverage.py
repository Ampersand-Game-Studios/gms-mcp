...
class TestEntrypoints(unittest.TestCase):
    ...
    def test_bootstrap_server_main_success_and_missing_dependency(self):
        ...
        def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
            if name.endswith("gamemaker_mcp_server"):
                raise ModuleNotFoundError("/Users/private/missing-mcp")
    ...
    def test_gamemaker_mcp_server_build_and_main(self):
        ...
        run_server.reset_mock(side_effect=True)
        run_server.side_effect = RuntimeError("/Users/private/runtime-state")
        stderr = io.StringIO()
        with (
            patch(
                "gms_mcp.gamemaker_mcp_server.build_server",
            ...
            result = gamemaker_mcp_server.main()
        self.assertEqual(result, 1)
        self.assertIn("host details were withheld", stderr.getvalue())
        self.assertNotIn("/Users/private", stderr.getvalue())
        ...
        with (
            patch(
                "gms_mcp.gamemaker_mcp_server.build_server",
                side_effect=FileNotFoundError("/Users/private/studio/project.yyp"),
            ),
            patch.dict(os.environ, {"GMS_MCP_EXPOSE_HOST_DIAGNOSTICS": ""}, clear=False),
            redirect_stderr(stderr),
        ):
            result = gamemaker_mcp_server.main()
        self.assertEqual(result, 1)
        self.assertIn("approved GameMaker project is unavailable or unsafe", stderr.getvalue())
        self.assertNotIn("/Users/private", stderr.getvalue())
...
class TestServerHelpers(unittest.TestCase):
    ...
    def test_resources_register_and_return_json(self):
        ...
        policy = SimpleNamespace(
            ...
            authorize=lambda _requested: Path("/tmp/project"),
        )
        server_resources.register(
            mcp,
            project_access_policy=policy,
        ...
        with patch(
            "gms_helpers.introspection.build_project_index",
            side_effect=RuntimeError("/Users/private/project-index"),
        ):
            result = asyncio.run(mcp.resources["gms://project/index"]())
        self.assertIn("host details were withheld", result)
