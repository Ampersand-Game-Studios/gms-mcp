...
class TestSkillsSourceFiles(unittest.TestCase):
    ...
    def test_every_skill_example_matches_live_cli_or_mcp_schema(self):
        ...
        for skill_doc in skill_docs:
            ...
            if skill_doc.name == "release-promote.md":
                release_scripts = {
                    "scripts/run_mcp_tool_smoke.py",
                    "scripts/generate_quality_reports.py",
                    "scripts/run_real_gamemaker_smoke.py",
                    "scripts/verify_release_certification.py",
                }
                seen_release_scripts: set[str] = set()
                for block_match in re.finditer(
                    r"```(?:bash|sh|shell)\s*\n(.*?)```",
                    content,
                    flags=re.DOTALL | re.IGNORECASE,
                    ...
                    for line in _logical_shell_lines(block_match.group(1)):
                        for segment in _shell_segments(line):
                            while segment and re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*=.*", segment[0]):
                                segment = segment[1:]
                            if segment[:3] != ["uv", "run", "python3"] or len(segment) < 4:
                                continue
                            script = segment[3]
                            if script not in release_scripts:
                                continue
                            seen_release_scripts.add(script)
                            file_contract_count += 1
                            script_path = repository_root / script
                            if not script_path.is_file():
                                failures.append(f"{skill_doc.name}: missing release script {script}")
                                continue
                            ...
                            if help_result.returncode != 0:
                                failures.append(f"{skill_doc.name}: {script} --help failed")
                                continue
                            help_text = help_result.stdout + help_result.stderr
                            for option in (token for token in segment[4:] if token.startswith("--")):
                                if option not in help_text:
                                    failures.append(f"{skill_doc.name}: {script} does not accept {option}")
                            if script == "scripts/run_mcp_tool_smoke.py" and "--base-project" in segment:
                                base_index = segment.index("--base-project") + 1
                                if base_index >= len(segment):
                                    failures.append(f"{skill_doc.name}: --base-project has no path")
                                else:
                                    configured_base = segment[base_index]
                                    is_placeholder = configured_base.startswith(("/path/to/", "<"))
                                    ...
                                    if (
                                        ...
                                        and "--init-minimal-base" not in segment
                                    ):
                                        failures.append(
                                            f"{skill_doc.name}: missing local base project {configured_base!r}; "
                                            "use --init-minimal-base for a portable command"
                                        )
                if seen_release_scripts != release_scripts:
                    failures.append(
                        f"{skill_doc.name}: release script contract mismatch: "
                        f"expected {sorted(release_scripts)}, found {sorted(seen_release_scripts)}"
                    )
    ...
    def test_fresh_wheel_runs_skill_and_claude_bundle_setup_outside_repo(self):
        """Prove repository-only files are present and usable in the built wheel."""
        repository_root = self.skills_dir.parents[1]
        with tempfile.TemporaryDirectory() as temp_dir:
