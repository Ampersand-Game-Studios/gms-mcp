...
class PathValidationIssue:
    ...
    def __init__(
        self, asset_path: str, asset_name: str, referenced_folder: str, issue_type: str, severity: str = "error"
    ):
...
def validate_folder_paths(
    project_root: str = ".", strict_mode: bool = False, include_parent_folders: bool = False
    ...
    try:
        ...
        for resource in resources:
            ...
            if (
                asset_path
                and os.path.exists(os.path.join(project_root, asset_path))
                and "options" not in asset_path.lower()
            ):
                try:
