...
def remove_instance(room_name: str, instance_id: str):
    ...
    room_name = validate_resource_name(room_name, "room")
    instance_id = validate_resource_name(instance_id, "instance")
    room_path = _find_room_file(room_name)
    room_data = _load_room_data(room_path)
...
def modify_instance(room_name: str, instance_id: str, **kwargs):
    ...
    # Update properties
    if "x" in kwargs:
        found_inst["x"] = float(kwargs["x"])
    if "y" in kwargs:
        found_inst["y"] = float(kwargs["y"])
    if "scaleX" in kwargs:
        found_inst["scaleX"] = float(kwargs["scaleX"])
    if "scaleY" in kwargs:
        found_inst["scaleY"] = float(kwargs["scaleY"])
...
def handle_modify_instance(args):
    ...
    if args.scale_x is not None:
        props["scaleX"] = args.scale_x
    if args.scale_y is not None:
        props["scaleY"] = args.scale_y
    if args.rotation is not None:
        props["rotation"] = args.rotation
    if args.object is not None:
        props["object_name"] = args.object
