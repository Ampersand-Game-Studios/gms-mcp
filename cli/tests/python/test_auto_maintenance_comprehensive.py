...
class TestAutoMaintenanceComprehensive(unittest.TestCase):
    ...
    def _create_asset(self, asset_type, asset_name, create_files=True, valid_json=True):
        ...
        if asset_type == "objects":
            ...
            asset_content.update(
                {
                    "layers": [{"$GMImageLayer": "", "name": "default", "resourceType": "GMImageLayer"}],
                    "sequence": {"$GMSequence": ""},
                }
            )
        elif asset_type == "scripts":
            asset_content.update({"isCompatibility": False, "isDnD": False})
...
class TestDetectMultiAssetDirectories(TestAutoMaintenanceComprehensive):
    ...
    def test_multi_asset_directory_objects(self):
        ...
        for obj_name in ["o_player", "o_enemy"]:
            asset_content = {"$GMObject": "v1", "%Name": obj_name, "name": obj_name, "resourceType": "GMObject"}
    ...
    def test_multi_asset_directory_mixed_types(self):
        ...
        for script_name in ["script_a", "script_b", "script_c"]:
            asset_content = {"$GMScript": "v1", "%Name": script_name, "name": script_name, "resourceType": "GMScript"}
...
class TestMaintenanceResult(TestAutoMaintenanceComprehensive):
    ...
    def test_add_lint_issues(self):
        ...
        error_issue = LintIssue(severity="error", category="json", file_path="test.yy", message="Invalid JSON")

        warning_issue = LintIssue(
            severity="warning", category="structure", file_path="test2.yy", message="Missing optional field"
        )
    ...
    def test_set_comma_fixes(self):
        ...
        comma_data = [("file1.yy", True, "Valid JSON"), ("file2.yy", False, "Invalid JSON syntax")]
    ...
    def test_event_sync_stats(self):
        ...
        result.event_sync_stats = {"orphaned_found": 2, "missing_found": 1, "orphaned_fixed": 2, "missing_fixed": 1}
    ...
    def test_complex_flag_computation(self):
        ...
        warning_issue = LintIssue(severity="warning", category="style", file_path="test.yy", message="Minor issue")
        result.add_lint_issues([warning_issue])

        self.assertFalse(result.has_errors, "Warnings should not set has_errors")

        # Add errors
        error_issue = LintIssue(severity="error", category="syntax", file_path="test2.yy", message="Critical issue")
...
class TestPrintFunctions(TestAutoMaintenanceComprehensive):
    ...
    def test_print_maintenance_summary_with_errors(self):
        ...
        # Add lint issues
        error_issue = LintIssue(severity="error", category="json", file_path="test.yy", message="Critical error")
        result.add_lint_issues([error_issue])
    ...
    def test_print_event_validation_report_with_issues(self):
        ...
        validation_issues = {"o_player": {"errors": ["Missing Create event"]}, "o_enemy": {"warnings": ["Style issue"]}}
    ...
    def test_print_orphan_cleanup_report_all_scenarios(self):
        ...
        stats = {"total_deleted": 5, "deleted_directories": ["dir1", "dir2"]}
...
class TestAutoMaintenanceEdgeCases(TestAutoMaintenanceComprehensive):
    ...
    def test_run_auto_maintenance_with_import_errors(self):
        ...
        self.assertIsInstance(
            result, MaintenanceResult, "Should return MaintenanceResult even with potential import issues"
        )
    ...
    def test_maintenance_with_corrupted_project_file(self):
        ...
        self.assertTrue(
            result.has_errors or result.degraded_mode, "Corrupted project file should cause errors or degraded mode"
        )
...
class TestAutoMaintenanceStressScenarios(TestAutoMaintenanceComprehensive):
    ...
    def test_maintenance_with_all_error_types(self):
        ...
        self.assertTrue(
            result.has_errors or len(result.json_issues) > 0 or len(result.orphaned_assets) > 0,
...
class TestAutoMaintenanceFullCoverage(unittest.TestCase):
    ...
    def setUp(self):
        ...
        project_data = {"resources": [], "folders": []}
    ...
    def test_import_error_handlers(self):
        ...
        mock_modules = {"maintenance.event_sync": MagicMock(), "maintenance.clean_unused_assets": MagicMock()}

        # Create mock functions
        mock_sync = MagicMock(
            return_value={"orphaned_found": 0, "missing_found": 0, "orphaned_fixed": 0, "missing_fixed": 0}
        )
