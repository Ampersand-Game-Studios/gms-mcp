...
def _safe_project_path(project_root: Path, raw_path: str | Path) -> Optional[Path]:
...
def _safe_project_relative(project_root: Path, raw_path: str | Path) -> Optional[str]:
    candidate = _safe_project_path(project_root, raw_path)
...
def _find_yyp(project_root: Path) -> Optional[Path]:
    ...
    try:
        yyp_files = [
            safe_path
            for candidate in sorted(project_root.glob("*.yyp"))
            if (safe_path := _safe_project_path(project_root, candidate.name)) is not None and safe_path.is_file()
        ]
        if not yyp_files:
            return None
        return yyp_files[0]
...
def list_assets_by_type(
    ...
    for res in yyp_data.get("resources", []):
        ...
        safe_path = _safe_project_relative(project_root, path)
        if safe_path is None:
            continue

        inferred_type = _infer_asset_type(safe_path)
        ...
        if folder_filter and not safe_path.lower().startswith(folder_filter):
            # Also check if it's just inside a folder of that name
            if folder_filter not in safe_path.lower():
                continue

        if inferred_type not in assets:
            assets[inferred_type] = []

        assets[inferred_type].append({"name": name, "path": safe_path, "type": inferred_type})
    ...
    if include_included_files and (not asset_type_filter or asset_type_filter == "includedfile"):
        ...
        if included_files:
            ...
            for inc_file in included_files:
                ...
                safe_path = _safe_project_relative(project_root, full_path)
                if safe_path is None:
                    continue
...
def get_asset_yy_path(project_root: Path, asset_identifier: str) -> Optional[Path]:
    ...
    if "/" in asset_identifier or "\\" in asset_identifier or asset_identifier.endswith(".yy"):
        p = _safe_project_path(project_root, asset_identifier)
        if p is None:
            return None
    ...
    for res in yyp_data.get("resources", []):
        ...
        if res_id.get("name") == asset_identifier:
            path = res_id.get("path")
            if path:
                full_path = _safe_project_path(project_root, path)
    ...
    for pattern_glob in file_patterns:
        ...
        try:
            for file_path in project_root.glob(pattern_glob):
                ...
                safe_file_path = _safe_project_path(project_root, file_path.relative_to(project_root))
                    ...
                    with open(safe_file_path, "r", encoding="utf-8", errors="ignore") as f:
                        for i, line in enumerate(f):
                            ...
                            if match:
                                results.append(
                                    {
                                        "file": safe_file_path.relative_to(project_root.resolve()).as_posix(),
                                        "line": i + 1,
                                        "match": match.group(0),
...
def build_asset_graph(project_root: Path, *, deep: bool = False, include_code_refs: bool = False) -> Dict[str, Any]:
    """
    ...
    for asset_type, asset_list in assets_by_type.items():
        for asset in asset_list:
            nodes.append({"id": asset["name"], "type": asset["type"], "path": asset["path"]})
    ...
    return {"nodes": nodes, "edges": edges, "node_count": len(nodes), "edge_count": len(edges), "deep_scan": use_deep}
...
def _scan_gml_references(project_root: Path, asset_names: Dict[str, str], add_edge):
    ...
    for gml_path in gml_files:
        ...
        safe_gml_path = _safe_project_path(project_root, gml_path.relative_to(project_root))
        if safe_gml_path is None or not safe_gml_path.is_file():
            continue

        try:
            content = safe_gml_path.read_text(encoding="utf-8", errors="ignore")

            # Determine the source asset (the file doing the referencing)
            rel_path = safe_gml_path.relative_to(project_root.resolve()).as_posix()
