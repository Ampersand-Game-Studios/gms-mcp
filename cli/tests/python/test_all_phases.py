...
class GameMakerCLITester:
        ...
        for cmd in commands:
            ...
            if success and "usage:" in stdout.lower():
                ...
                self.result.add_failure(
                    f"Help command: {cmd.split('/')[-1].split()[1]}", f"Return code or missing usage. stderr: {stderr}"
                )
    ...
    def test_asset_creation(self):
        ...
        folder_test_cases = [("folder", "Test Folder", "--path 'folders/Test Folder.yy'")]
        ...
        for asset_type, asset_name, extra_args in test_cases:
            ...
            if success:
                ...
                self.result.add_failure(f"Create {asset_type}: {asset_name}", f"Command failed. stderr: {stderr}")
    ...
    def test_maintenance_commands(self):
        ...
        for maint_cmd in maintenance_commands:
            ...
            if success:
                ...
                self.result.add_failure(f"Maintenance: {maint_cmd}", f"Command failed. stderr: {stderr}")
    ...
    def test_room_management(self):
        ...
        for cmd in layer_commands:
            ...
            if success:
                ...
                self.result.add_failure(f"Room layer: {cmd.split()[-2]}", f"Command failed. stderr: {stderr}")
    ...
    def test_event_management(self):
        ...
        for cmd in event_commands:
            ...
            if success:
                ...
                self.result.add_failure(
                    f"Event: {cmd.split()[-2]} {cmd.split()[-1]}", f"Command failed. stderr: {stderr}"
                )
...
class TestAllPhases:
    ...
    def test_all_phases_integration(self):
        ...
        success_rate = (
            (tester.result.tests_passed / tester.result.tests_run * 100) if tester.result.tests_run > 0 else 0
        )
